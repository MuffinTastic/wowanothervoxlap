...
const char *prognam = "SDLMain App (by KS/JF)";
long progresiz = 1;
...
long xres = 640, yres = 480, colbits = 32, fullscreen = 1, maxpages = 8;
...
long initdirectdraw(long daxres, long dayres, long dacolbits)
{
	...
	SDL_WM_SetCaption(prognam, "game");
...
#define MINBREATHREND 2048  //(256) Keep this high to avoid clicks, but much smaller than SNDSTREAMSIZ!
#define LSAMPREC 12        //(10) No WAV file can have more than 2^(31-LSAMPREC) samples
#define MAXPLAYINGSNDS 512 //(256)
#define VOLSEPARATION 0.7f  //(1.f) Range: 0 to 1, .5 good for headphones, 1 good for speakers
#define EARSEPARATION 8.f  //(8.f) Used for L/R sample shifting
#define SNDMINDIST 0.5f    //(32.f) Shortest distance where volume is max / volume scale factor
#define SNDMAXDIST 128.0f  //(128.0f) Maximum distance for sound
...
void playsound (const char *filnam, long volperc, float frqmul, void *pos, long flags)
{
	...
	if (flags&KSND_3D)
	{
		...
		if (f <= SNDMINDIST*SNDMINDIST) { f = SNDMINDIST; h = (float)ivolsc; }
		else if (f >= SNDMAXDIST*SNDMAXDIST) { f = SNDMAXDIST; ivolsc = 0; h = (float)ivolsc; }
		else { f = sqrt(f) * 2.0 /*changed from 1*/; h = ((float)ivolsc)*SNDMINDIST / f; }
		g /= f;  //g=-1:pure left, g=0:center, g=1:pure right
			//Should use exponential scaling to keep volume constant!
...
long progactive = 1;
void muffin_setactive(long active)
{
	SDL_GrabMode g;

	if (active != mouse_acquire) {
		g = SDL_WM_GrabInput(active ? SDL_GRAB_ON : SDL_GRAB_OFF);
		mouse_acquire = kbd_acquire = (g == SDL_GRAB_ON);
...
void setalwaysactive(long active)
{
	alwaysactive = active;
}
...
long keyread ()
{
	...
	if (keybufr == keybufw2) return(0); // ??? - muffin
