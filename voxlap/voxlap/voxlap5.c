...
void setcamera_float (point3d *ipo, point3d *ist, point3d *ihe, point3d *ifo,
...
void clipmove_float(point3d *p, point3d *v, float acr)
{
	double f, gx, gy, gz, nx, ny, nz, ex, ey, ez, hitx, hity, hitz, cr;
	//double nx2, ny2, nz2, ex2, ey2, ez2; //double ox, oy, oz;
	long i, j, k;
...
void hitscan_f(point3d *pf, point3d *df, lpoint3d *h, long **ind, long *dir)
{
	dpoint3d pd, dd, *p = &pd, *d = &dd;
	pd.x = pf->x; pd.y = pf->y; pd.z = pf->z;
	dd.x = df->x; dd.y = df->y; dd.z = df->z;
...
void hitscan_side(lpoint3d *p, long dir)
{
	switch (dir) {
	case 0:
		p->x--;
...
void sprhitscan_f(point3d *p0, point3d *v0, vx5sprite *spr, lpoint3d *h, kv6voxtype **ind, float *vsc)
{
	kv6voxtype *vx[4];
	kv6data *kv;
	point3d t, u, v;
	...
	t.y = (float)kv->ysiz; (*(long *)&t.y)--;
	t.z = (float)kv->zsiz; (*(long *)&t.z)--;
	if (u.x <   g) { if (v.x <= 0) return; f = (g - u.x) / v.x; u.x = g; u.y += v.y*f; u.z += v.z*f; } else if (u.x > t.x) { if (v.x >= 0) return; f = (t.x - u.x) / v.x; u.x = t.x; u.y += v.y*f; u.z += v.z*f; }
	if (u.y <   0) { if (v.y <= 0) return; f = (0 - u.y) / v.y; u.y = 0; u.x += v.x*f; u.z += v.z*f; } else if (u.y > t.y) { if (v.y >= 0) return; f = (t.y - u.y) / v.y; u.y = t.y; u.x += v.x*f; u.z += v.z*f; }
	if (u.z <   0) { if (v.z <= 0) return; f = (0 - u.z) / v.z; u.z = 0; u.x += v.x*f; u.y += v.y*f; } else if (u.z > t.z) { if (v.z >= 0) return; f = (t.z - u.z) / v.z; u.z = t.z; u.x += v.x*f; u.y += v.y*f; }
	...
	ftol(u.y - .5, &a.y); if ((unsigned long)a.y >= kv->ysiz) return;
	ftol(u.z - .5, &a.z); if ((unsigned long)a.z >= kv->zsiz) return;
	if (*(long *)&v.x < 0) { d.x = -1; u.x -= a.x;      v.x *= -g; } else { d.x = 1; u.x = a.x + 1 - u.x; v.x *= g; }
	if (*(long *)&v.y < 0) { d.y = -1; u.y -= a.y;      v.y *= -g; } else { d.y = 1; u.y = a.y + 1 - u.y; v.y *= g; }
	if (*(long *)&v.z < 0) { d.z = -1; u.z -= a.z;      v.z *= -g; } else { d.z = 1; u.z = a.z + 1 - u.z; v.z *= g; }
	ftol(u.x*v.z - u.z*v.x, &p.x); ftol(v.x, &q.x);
	ftol(u.y*v.z - u.z*v.y, &p.y); ftol(v.y, &q.y);
	ftol(u.y*v.x - u.x*v.y, &p.z); ftol(v.z, &q.z);
...
void loadnul (point3d *ipo, point3d *ist, point3d *ihe, point3d *ifo)
{
	lpoint3d lp0, lp1;
	long i, x, y;
	char *v;
...
long loadvxl (const char *lodfilnam) // , dpoint3d *ipo, dpoint3d *ist, dpoint3d *ihe, dpoint3d *ifo)
{
	...
	v = (unsigned char *)(&vbuf[1]); //1st dword for voxalloc compare logic optimization
	leng = fsiz-kztell();
	kzread((void *)v,leng);
...
long loadvxl_new(const char *lodfilnam, dpoint3d *ipo, dpoint3d *ist, dpoint3d *ihe, dpoint3d *ifo)
{
	FILE *fil;
	long i, j, fsiz;
	unsigned char *v, *v2;
...
long loadvxlbytes(const char *lodbytes, long fsiz)
{
	long i, j;
	unsigned char *v, *v2;
	long leng;
	...
	if (lodbytes == NULL || fsiz == 0)
		return 0;

	/*memcpy(ipo, lodbytes + 12, 24);
	memcpy(ipo, lodbytes + 36, 24);
	memcpy(ipo, lodbytes + 60, 24);
	memcpy(ipo, lodbytes + 84, 24);
...
long savevxl_new(const char *savfilnam, dpoint3d *ipo, dpoint3d *ist, dpoint3d *ihe, dpoint3d *ifo)
{
	FILE *fil;
	long i;
...
#if (USEZBUFFER == 1)
void drawsquarefill(float px, float py, float pz, float siz, long col)
{
	float ox, oy, oz;
	float tx, ty, r, s;
	...
	// treat siz as 'diameter'
	s = fabsf(siz)/2.0f;

	// transform point from world space to local space
	ox = px - gipos.x; oy = py - gipos.y; oz = pz - gipos.z;
	pz = ox*gifor.x + oy*gifor.y + oz*gifor.z; if (pz < SCISDIST) return;
	px = ox*gistr.x + oy*gistr.y + oz*gistr.z;
	py = ox*gihei.x + oy*gihei.y + oz*gihei.z;

	r = 1.0f / pz; // perspective scalar

	// transform from local space to screen space (window pixel coordinates)
	...
	if (minx > xres || miny > yres) return;
	if (maxx < 0 || maxy < 0) return;

	// if within view, clamp corners to window space
	if (minx < 0) minx = 0;
	if (miny < 0) miny = 0;
	if (maxx > xres) maxx = xres;
	if (maxy > yres) maxy = yres;

	// if fog enabled, blend with fog color based on distance
	if (ofogdist >= 0)
	{
		// use cylindrical x-y distance for fog
		ftol(sqrt(ox*ox+oy*oy), &t);
		if (t > 2047) t = 2047;
		t = (long)(*(short*)&foglut[t]);
		col = (((((vx5.fogcol & 255) - (col & 255)) * t) >> 15)) +
			((((((vx5.fogcol >> 8) & 255) - ((col >> 8) & 255)) * t) >> 15) << 8) +
			((((((vx5.fogcol >> 16) & 255) - ((col >> 16) & 255)) * t) >> 15) << 16) + col;
	}

	p = ylookup[miny] + frameplace;
	maxyp = ylookup[maxy] + frameplace;

	while (1)
	{
		if (p >= maxyp) break; // must be at start of loop to prevent crash
		if (maxx > xres) maxx = xres;
		for (x=minx;x<maxx;x++)
		{
			if (*(long *)&pz < *(long *)(p + (x << 2) + zbufoff))	// compare against zbuf
			{
				*(long*)(p + (x << 2) + zbufoff) = *(long*)&pz;		// set zbuf
				*(long*)(p + (x << 2)) = col;						// set color
			}
		}
		miny++;
		p += bytesperline;
...
void drawpicinquadalpha(long rpic, long rbpl, long rxsiz, long rysiz,
	long wpic, long wbpl, long wxsiz, long wysiz,
	float x0, float y0, float x1, float y1,
	float x2, float y2, float x3, float y3,
	long tint, long spec)
	...
	while (1)
	{
		...
		for(i=0;i<4;i++) //26-connectivity
		{
			...
			while (1)
			{
				...
				if (ov[1] > z1) continue;
				if (z0 > v[3]) continue; // good

				j = isnewfloatingot((long)ov);
				if (j < 0)
				{
					// Add current span to floating section
					isnewfloatingadd((long)ov);
					if (vlstcnt >= VLSTSIZ) return(0); //EVIL HACK TO PREVENT CRASH!
