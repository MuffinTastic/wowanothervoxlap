...
void drawsquarefill(float px, float py, float pz, float siz, long col)
{
	...
	r = 1.0f / pz;
	px = (ox*gistr.x + oy*gistr.y + oz*gistr.z)*gihz;
	py = (ox*gihei.x + oy*gihei.y + oz*gihei.z)*gihz;

	x = px*r + gihx - .5f; if ((unsigned long)x >= (unsigned long)xres) return;
	y = py*r + gihy - .5f; if ((unsigned long)y >= (unsigned long)yres) return;

	if (ofogdist >= 0) //If fog enabled...
	{
		ftol(sqrt(px*px + py*py), &t); //Use cylindrical x-y distance for fog
		if (t > 2047) t = 2047;
	...
	if (*(long *)&pz < *(long *)(p + (x << 2) + zbufoff)) {
		*(long *)(p + (x << 2) + zbufoff) = *(long *)&pz;
		*(long *)(p + (x << 2)) = col;
	}
}
#endif
...
long isnewfloating (flstboxtype *flb)
{
	...
	while (1)
	{
		...
		for(i=0;i<4;i++) //26-connectivity // or 6???
		{
			switch(i)
			{
				...
				//case 5: nx = p.x+1; ny = p.y-1; break; // or 6???
			...
			while (1)
			{
				...
				//if ((long)ov != flb->chk.z) continue; // 6-connectivity?
