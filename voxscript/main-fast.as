...
const double SHOOTDELAY = 0.1; // 1.5;
...
{
	int sz;
	if (x < 0.0f || x >= 512.0f || y < 0.0f || y >= 512.0f) return true;
    if (z < 0.0f) return false;
	sz = int(z);
	if (sz == 63) sz = 62;
	else if (sz >= 63) return true;
	else if (sz < 0) return false;
    if (sz == 63) return isvoxelsolid(x, y, sz) == 1;
